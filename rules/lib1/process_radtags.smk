...
rule process_radtags:
    input:
        in1="results/lib1/clone_filter/{sample}_1.1.fq.gz",
        in2="results/lib1/clone_filter/{sample}_2.2.fq.gz",
    output:
        R1extraones="results/lib1/process_radtags/{sample}_1.1.1.fq.gz",
        R1rem="results/lib1/process_radtags/{sample}_1.1.rem.1.fq.gz",
        R2extratwos="results/lib1/process_radtags/{sample}_2.2.2.fq.gz",
        R2rem="results/lib1/process_radtags/{sample}_2.2.rem.2.fq.gz"
    conda:
        "../../envs/stacks.yaml"
    params:
    ...
    resources:
        ...
        time="12:00:00"
    log:
        "results/lib1/logs/process_radtags/{sample}.log"
    benchmark:
        "results/lib1/benchmarks/process_radtags/{sample}.bmk"
    shell:
        " (process_radtags -i gzfastq --threads 10  "
        " -1 {input.in1} -2 {input.in2}  "
        " -o results/lib1/process_radtags/  "
